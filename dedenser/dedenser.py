...
class Dedenser:
    ...
    def __init__(
        ...
        if target > 1 or target < 0:
            raise ValueError(
                f"Target must be between 0 & 1.\
                               User provided {target}"
            )
        if data.shape[1] != 3:
            if show:
                raise ValueError(
                    "Visualization with high dimensional data is not supported."
                )
        ...
        self.Weighted = False
        if self.v_weight is not None or self.d_weight is not None:
            self.Weighted = True
        self.Show = show
        self.GUI = GUI
    ...
    def start(self):
        """Performs clustering and calculates cluster volumes."""
        hdbscan = HDBSCAN(
            min_cluster_size=self.min_size, cluster_selection_epsilon=self.epsilon
        )
        ...
        for cluster in range(0, max(self.clusters) + 1):
            c_points = [
                self.data[i] for i, x in enumerate(self.clusters) if x == cluster
            ]
            if self.alpha:
        ...
        if self.Show:
            ...
            c_points = np.array(
                [self.data[i] for i, x in enumerate(self.clusters) if x == -1]
            )
            if len(c_points) != 0:
                ax.scatter(c_points[:, 0], c_points[:, 1], c_points[:, 2], s=0.3, c="k")
            for cluster in range(0, max(self.clusters) + 1):
                c_points = np.array(
                    [self.data[i] for i, x in enumerate(self.clusters) if x == cluster]
                )
                ax.scatter(c_points[:, 0], c_points[:, 1], c_points[:, 2], s=0.3)
            plt.tight_layout()
    ...
    def check_state(self):
        ...
        """
        if self.n_target * 0.95 < len(self.keep_list):  # can make a variable threshold
    ...
    def thinner(self):
        ...
        Check_Targs = True
        while Check_Targs:
            Check_Targs, c_is, c_pointses, cds = self.manage_targets(
                c_is, c_pointses, cds
            )
        o_targ = 0
        for cvol, cind, points, cd in zip(self.vols, c_is, c_pointses, cds):
            ...
            if self.Weighted:
                if self.d_weight is not None:
                    ...
                    vt = sum(
                        [
                            np.exp(w * (cvols / sum(self.vols) - 1))
                            for cvols in self.vols
                        ]
                    )
    ...
    def manage_targets(self, c_is, c_pointses, cds):
        ...
        with fewer points than their target, and removing them from the running list
        of clusters.
        """
        New_Targs = False
    ...
    def downsample(self):
        ...
        If the state is `'noisy'`, the noise from HDBSCAN is downsampled based on its
        volume prior to the downsampling of clusters using the `thinner()` method.
        ...
        if self.state == "noisy":
            ...
            no_targ = round(noise_v / (noise_v + sum(self.vols)) * self.n_target * 0.8)
            idx = pcu.downsample_point_cloud_poisson_disk(
                np.array(noise),
                -1,
                no_targ,
        ...
        if self.Show:
            ...
            c_points = np.array(
                [
                    ...
                    if x == -1 and i in self.keep_list
                ]
            )
            if len(c_points) != 0:
                ax.scatter(c_points[:, 0], c_points[:, 1], c_points[:, 2], s=0.3, c="k")
            for cluster in range(0, max(self.clusters) + 1):
                c_points = np.array(
                    [
                        self.data[i]
                        for i, x in enumerate(self.clusters)
                        if x == cluster and i in self.keep_list
                    ]
                )
                if len(c_points) != 0:
                    ax.scatter(c_points[:, 0], c_points[:, 1], c_points[:, 2], s=0.3)
