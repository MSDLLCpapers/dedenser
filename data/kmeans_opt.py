...
np.random.seed(100)
print('imports done')
class Numeric_Assistant_BHC(object):
...
paddy_sampler = Numeric_Assistant_BHC(observations=observations,repeats=True,raw_responce=True)
...
kmeans.fit(np.array(observations)[:,:-1])
centroids = kmeans.cluster_centers_

distances = distance.cdist(np.array(observations)[:,:-1], centroids, 'euclidean')
dd = np.argmin(distances,axis=0)

np.save('kmeans_ind',dd)
...
for repeat in range(25):
    ...
    try:    
        runner = paddy.PFARunner(space=rs,eval_func=dummy_sample,paddy_type='population',rand_seed_number=99,yt=5,Qmax=5,r=.1,iterations=1)
        runner.run_paddy(file_name='temp',verbose='pop')
    except:
        print()
